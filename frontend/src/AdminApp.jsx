...
const toForm = (course = emptyCourse) => ({
  ...
  website: course.resources?.website || "",
...
function AdminApp() {
  // Starts empty every time: admin must log in again after refresh.
  ...
  const request = async (path) => {
    return axios.get(`${API}/api/admin${path}`, {
      headers
    });
  };
  ...
  const loadAll = async () => {
    try {
      const [dashboard, userList, courseList] = await Promise.all([
        request("/dashboard"),
        request("/users"),
        request("/courses")
      ]);
  ...
  const login = async (event) => {
    ...
    try {
      ...
      // Token is only kept while this page remains open.
  ...
  const saveCourse = async (event) => {
    ...
    try {
      ...
      const response = editingId
        ? await axios.put(
            `${API}/api/admin/courses/${editingId}`,
            payload,
            { headers }
          )
        : await axios.post(
      ...
      <main className="admin-main">
        ...
        {tab === "overview" && (
          <>
            <section className="stat-grid">
              {[
                ["Learners", data.summary.users || 0],
                ["Courses", data.summary.courses || 0],
                ["Recommendations", data.summary.recommendations || 0]
              ].map(([label, value]) => (
        ...
        {tab === "users" && (
          <section className="panel">
            <h2>
              All learners <span>{users.length}</span>
            </h2>
                    ...
                    <tr key={user._id}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() =>
                            remove("users", user._id, user.name)
                          }
                        >
                          Delete
                        </button>
              ...
              {courses.map((course) => (
                <article className="admin-course" key={course._id}>
                  <div>
                    ...
                    <small>
                      {course.level} · {course.duration || "No duration"}
                    </small>
                  </div>
                  ...
                  <div className="course-actions">
                    ...
                    <button
                      className="danger-link"
                      onClick={() =>
                        remove("courses", course._id, course.title)
                      }
                    >
                      Delete
                    </button>
